...
function drawRadar() {
    ...
    const sx = cx + Math.cos(Math.PI + sweep) * radius;
    const sy = cy + Math.sin(Math.PI + sweep) * radius;

    ctx.strokeStyle = "rgba(140,255,154,0.65)";
    ctx.lineWidth = 2;
    ...
    if (distance > 0 && distance <= 200) {
        ...
        ctx.strokeStyle = "rgba(140,255,154,0.25)";
        ctx.stroke();
    }
